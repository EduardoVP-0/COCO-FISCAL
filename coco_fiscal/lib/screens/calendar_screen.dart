...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Future<void> obtenerEventosDelDia(DateTime fecha) async {
    ...
    final url = Uri.parse(
      "http://10.0.2.2:81/api_coco/get_events.php?fecha=$fechaStr",
    );
  ...
  Future<List<Map<String, dynamic>>> obtenerProximosEventos() async {
    final url = Uri.parse(
      "http://10.0.2.2:81/api_coco/get_upcoming_events.php",
    );
    final response = await http.get(url);
  ...
  Future<void> obtenerTodosLosEventos() async {
    final url = Uri.parse("http://10.0.2.2:81/api_coco/get_all_events.php");
    final response = await http.get(url);
  ...
  Future<void> eliminarEvento(String id) async {
    final url = Uri.parse(
      "http://10.0.2.2:81/api_coco/delete_event.php?id=$id",
    );
